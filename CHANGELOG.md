...
- MCP App iframes (`dag_graph_ui`, `run_heatmap_ui`) now load the SDK via `https://esm.sh/@modelcontextprotocol/ext-apps@1.7.1?bundle` so all transitive deps (notably zod) are inlined into a single response. Without `?bundle`, the SDK module loaded but its `zod` import resolved from an esm.sh subdomain not in the iframe's CSP `connect-src`, leaving the schema layer half-initialised and throwing `TypeError: t.custom is not a function` at `types.ts:22` the moment the SDK called `zod.custom(...)`. The heatmap and DAG graph iframes therefore stayed at "Waiting for tool output..." indefinitely even though the resource was being served correctly.
